...
def FinalFunction(inputURLbyUser):
    ...
    def findNumberOfReviewsPerStar(urlx,starx):
        urlx+="ref=cm_cr_unknown?ie=UTF8&filterByStar={star}_star&reviewerType=all_reviews&pageNumber={page}#reviews-filter-bar".format(star=starx,page="1")
    ...
    def extractReviewsIntoText(urlxy):
        ...
        if oneStarNumberPages<3:
            for i in range(1,oneStarNumberPages+1):
                urlxyz = urlxy
                urlxyz+="ref=cm_cr_unknown?ie=UTF8&filterByStar={star}_star&reviewerType=all_reviews&pageNumber={page}#reviews-filter-bar".format(star="one",page=str(i))
        ...
        if twoStarNumberPages!=0:
            urlxyz = urlxy
            urlxyz+="ref=cm_cr_unknown?ie=UTF8&filterByStar={star}_star&reviewerType=all_reviews&pageNumber={page}#reviews-filter-bar".format(star="two",page="1")
            soupxy = html_code(urlxyz)
            rev2=""
            for j in cus_rev(soupxy):
                twoStarReviews[0]+=j
        ...
        if threeStarNumberPages!=0:
            urlxyz = urlxy
            urlxyz+="ref=cm_cr_unknown?ie=UTF8&filterByStar={star}_star&reviewerType=all_reviews&pageNumber={page}#reviews-filter-bar".format(star="three",page="1")
            soupxy = html_code(urlxyz)
            rev2=""
            for j in cus_rev(soupxy):
        ...
        if fourStarNumberPages!=0:
            urlxyz = urlxy
            urlxyz+="ref=cm_cr_unknown?ie=UTF8&filterByStar={star}_star&reviewerType=all_reviews&pageNumber={page}#reviews-filter-bar".format(star="four",page="1")
            soupxy = html_code(urlxyz)
            rev2=""
            for j in cus_rev(soupxy):
                fourStarReviews[0]+=j
        ...
        if fiveStarNumberPages<3:
            for i in range(1,fiveStarNumberPages+1):
                urlxyz = urlxy
                urlxyz+="ref=cm_cr_unknown?ie=UTF8&filterByStar={star}_star&reviewerType=all_reviews&pageNumber={page}#reviews-filter-bar".format(star="five",page=str(i))
                soupxy = html_code(urlxyz)
                rev2=""
                for j in cus_rev(soupxy):
                    fiveStarReviews[0]+=j
    ...
    def gpt3_completion(prompt, engine='text-davinci-003', temp=0.7, top_p=1.0, tokens=1000, freq_pen=0.5, pres_pen=0.5, stop=['<<END>>']):#here, tokens is token limit
        max_retry = 5
        retry = 0
        while True:
            try:
    ...
    def beautifySummary(summary):
        ...
        strengths = a1[1].split("Weaknesses: ")[0]
        weaknesses = a1[1].split("Weaknesses: ")[1].split("Functionality: ")[0]
        functionality = a1[1].split("Weaknesses: ")[1].split("Functionality: ")[1].split("Worthiness: ")[0]
        worthiness = a1[1].split("Weaknesses: ")[1].split("Functionality: ")[1].split("Worthiness: ")[1]
    ...
    threeStarNumberPages = findNumberOfReviewsPerStar(model_input_url,"three")//10
    fourStarNumberPages = findNumberOfReviewsPerStar(model_input_url,"four")//10
    fiveStarNumberPages = findNumberOfReviewsPerStar(model_input_url,"five")//10
...
SUMMARY HIGHLIGHTING KEY STRENGTHS, WEAKNESSES, FUNCTIONALITY, WORTHINESS: """
    ...
    for chunk in chunks:
        count = count + 1
        prompt = promptText1.replace('<<SUMMARY>>', chunk) #open_file('prompt.txt').replace('<<SUMMARY>>', chunk)
...
NON-REDUNDANT AND NON-CONFLICTING SUMMARY HIGHLIGHTING KEY STRENGTHS, WEAKNESSES, FUNCTIONALITY, WORTHINESS: """
    ...
    if (len(result)*0.25) < 3000:
        summaryFinal = gpt3_completion(promptText2.replace('<<SUMMARY>>',result))
        # print()
        ...
        for result_chunk in result_chunks:
            result_chunk_summary = gpt3_completion(promptText2.replace('<<SUMMARY>>',result_chunk))
            result2.append(result_chunk_summary)
        result2 = '\n\n'.join(result2)
        summaryFinal = gpt3_completion(promptText2.replace('<<SUMMARY>>',result2))
        print()
...
x = FinalFunction("https://www.amazon.in/New-Apple-iPhone-12-64GB/dp/B08L5WHFT9/ref=sr_1_3?keywords=iphone+12&qid=1673859500&sprefix=i%2Caps%2C445&sr=8-3")
print(x)
